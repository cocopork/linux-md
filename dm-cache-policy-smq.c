 ...
 */
// czs：entry_space 管理一组 entry，clean 和 dirty 队列共享 entry
struct entry_space {
	struct entry *begin;
	struct entry *end;
};
 ...
 */
// czs：stochastic-multi-queue 是一组 LRU（最近最少使用）列表，按照级别进行堆叠。条目在使用时会向上移动到更高的级别，这样可以大致按照访问频率将最常访问的条目放在顶部级别，最不常访问的放在底部级别
 ...
 */
// czs: 从低级到高级找到首个可用的非哨兵元素，即找到最低级别的最老元素
...
// czs：重新分配队列（queue）中各个层级的元素，以满足每个层级的目标数量（target_count）要求
		      ...
		      struct entry *s1, struct entry *s2)
{
	...
	if (extra_levels && (e->level < q->nr_levels - 1u)) {
		// czs：从当前层级的下一层级开始遍历，找到第一个非哨兵条目，将其与当前条目交换
...
// czs：根据哈希桶号获取对应的entry space 中条目，bucket 是哈希桶号，哈希表中存储的是块号，通过块号获取对应的 entry
 ...
 */
// czs：在哈希表中查找 oblock 对应的 entry，如果找到，将其移动到哈希桶的头部
...
static void __sentinels_init(struct smq_policy *mq)
{
	...
	// czs：逐层获取写回哨兵和降级哨兵，将哨兵层级设置为当前层，写回哨兵插入到脏块队列，降级哨兵插入到干净块队列
...
{
	// czs：设置下一次写回周期和下一次降级周期的时间戳
	mq->next_writeback_period = jiffies + WRITEBACK_PERIOD;
	mq->next_demote_period = jiffies + DEMOTE_PERIOD;

	// ggboy:先将上次哨兵加入队列，在将当前哨兵加入队列。
...
static void requeue(struct smq_policy *mq, struct entry *e)
{
	...
	// czs：条目 e 对应缓存块在缓存命中位图中未设置时，如果条目 e 干净，则将其插入到干净块队列，否则插入到脏块队列
...
// czs：据缓存的命中和未命中次数，以及预定义的晋升级别表，计算出默认的晋升级别
...
static void update_promote_levels(struct smq_policy *mq)
{
	 ...
	 * eager to promote.
	 */
	// czs：如果缓存中有空闲条目，则将晋升级别阈值设置为热点队列层级数的一半，否则设置为默认的晋升级别（有空闲条目则快速提升）
	unsigned threshold_level = allocator_empty(&mq->cache_alloc) ?
		default_promote_level(mq) : (NR_HOTSPOT_LEVELS / 2u);

	// czs：阈值级别不能超过热点队列的层级数
	 ...
	 */
	// czs：根据热点队列的状态评估，更新晋升级别阈值，如果热点队列表现不好，则将阈值除以 4，表现一般则除以 2，threshold_level越大，promote_level越小，越容易提升
...
// czs：定是否应该将热点队列中的条目提升到缓存队列中
static enum promote_result should_promote(struct smq_policy *mq, struct entry *hs_e,
					  int data_dir, bool fast_promote)
{
	// czs: 如果数据方向为写，则如果缓存队列中有空闲条目并且使用快速提升，则返回 PROMOTE_TEMPORARY，表示临时提升；
	if (data_dir == WRITE) {
		if (!allocator_empty(&mq->cache_alloc) && fast_promote)
			return PROMOTE_TEMPORARY;
		// czs：否则根据热点条目的级别（hs_e->level）是否达到写入提升级别（write_promote_level），调用 maybe_promote 函数来确定是否应该进行提升操作。
		return maybe_promote(hs_e->level >= mq->write_promote_level);
	} else
		// czs：数据方向为读取，根据热点条目的级别（hs_e->level）是否达到读取提升级别（read_promote_level），调用 maybe_promote 函数来确定是否应该进行提升操作。
		return maybe_promote(hs_e->level >= mq->read_promote_level);
}

// czs：将给定的逻辑块号 b 转换为热点队列（hotspot queue）中的块号
...
static struct entry *update_hotspot_queue(struct smq_policy *mq, dm_oblock_t b)
{
	...
	if (e) {
		// czs: 热点队列命中后，对条目 e 进行重新排队，根据是否命中热点队列，设置是否跳级（之前未命中则增加级别）
...
/*----------------------------------------------------------------*/
// czs：在缓存中查找oblock对应的cblock，如果找到则返回0，否则返回-ENOENT，并更新热点队列，如果需要将oblock提升到缓存中，则将提升任务添加到后台工作队列中
		    ...
		    struct policy_work **work, bool *background_work)
{
	...
	*background_work = false;
	...
	// czs：在缓存哈希表中查找 oblock 对应的 entry
	e = h_lookup(&mq->table, oblock);
	if (e) {
		stats_level_accessed(&mq->cache_stats, e->level);

		// czs：缓存命中后，对条目 e 进行重新排队
		requeue(mq, e);
		// czs：根据条目 e 推测出缓存块号
		*cblock = infer_cblock(mq, e);
		...
		// czs: 判断是否需要将 oblock 提升到缓存中，提升条件为：热点队列中的条目达到提升级别（write_promote_level）或者缓存队列中有空闲条目并且使用快速提升
...
// czs： 从 SMQ 缓存策略中获取后台工作项。它通过调用后台跟踪器来获取工作项，并在需要时触发清理和写回操作以满足缓存策略的目标。
...
// czs：在 SMQ 缓存策略中加载给定的逻辑块和物理块的映射关系，并设置相应的属性；设置hint值则将hint值作为条目的级别，否则随机生成一个级别
...
// czs: smq 策略定期调用 tick 函数，更新热点队列和缓存队列的状态，更新哨兵状态
...
// czs：初始化策略函数，将策略函数指针指向 smq 策略函数
				...
				unsigned *nr_hotspot_blocks)
{
	...
	*nr_hotspot_blocks = max(nr_cache_blocks / 4u, 1024u);
	...
	// czs：如果热点块的块大小和块数量过大，则将热点块的块大小减半，直到热点块的块大小和块数量不再过大
	while ((*hotspot_block_size > cache_block_size) &&
	       too_many_hotspot_blocks(origin_size, *hotspot_block_size, *nr_hotspot_blocks))
		*hotspot_block_size /= 2u;
}

// czs: 创建 smq 策略, 输入参数为：缓存大小（缓存块数量）、源设备大小（扇区数）、缓存块大小（扇区数）、是否模拟消息队列、是否允许迁移（默认允许）
...
{
	unsigned i;
	// czs：设置每个队列的哨兵数量为 2 倍缓存层级数；有两个哨兵队列，分别用于写回和降级
	...
	// czs：初始化热点块分配器，起始位置哨兵总数据量，结束位置为哨兵总数据量加上热点块数量，即热点块条目在 entry space 中紧接着哨兵条目
	init_allocator(&mq->hotspot_alloc, &mq->es, total_sentinels,
		       total_sentinels + mq->nr_hotspot_blocks);

	// czs：初始化缓存块分配器，缓存块条目在 entry space 中紧接着热点块条目
	...
	// czs：初始化热点队列，热点队列的顶部层级数为 8，每个顶部层级中的热点块数量为热点块总数量除以 8，或者缓存块数量除以热点块数量，取较小值
	//? ggboy: top_levels的作用以及nr_in_top_levels数量的确定是否有讲究？
	...
	// czs：设置下一次热点块和缓存块的时期为当前时间戳
	mq->next_hotspot_period = jiffies;
	mq->next_cache_period = jiffies;

	// czs：创建后台任务跟踪器，大小为 4096（硬编码）
...
// czs: 注册四种缓存替换策略, smq, mq, cleaner, default(smq)
