...
struct cache {
	...
	// ggboy:migration_threshold 设置迁移阈值，也是判断缓存设备是否空闲的阈值
...
static bool writethrough_mode(struct cache *cache)queue_work(
{
...
/*----------------------------------------------------------------*/
// czs：锁定oblock对应的cell，如果锁定失败，返回false
...
static void mg_full_copy(struct work_struct *ws)
{
	...
	//? ggboy:这两个分支都涉及到 mg_upgrade_lock() 函数，
	// 前者是直接调用，后者则是挂在mg->k->ws->func上，
	// 似乎是要完成copy之后才会进行的收尾函数。
...
// czs: 创建cache 的参数 cache <metadata dev> <cache dev> <origin dev> <block size> <features> <policy>
...
static int cache_create(struct cache_args *ca, struct cache **result)
{
	...
	// ggboy:使用kcopyd的第一步是创建 kcopyd client。
	// ggboy:这里dm_kcopyd_throttle是一个限流器，尚不清楚具体作用
	...
	//? ggboy:bio prison在device mapper中的作用以及具体行为是什么？
	...
	// ggboy:初始化 struct dm_cache_migration 的内存池 
...
static int __init dm_cache_init(void)
{
	...
	// ggboy:创建迁移任务对象结构体dm_cache_migration的内存缓存
