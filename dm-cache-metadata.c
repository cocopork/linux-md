...
struct dm_cache_metadata {
	...
	struct list_head list;		/* 用于将元数据结构体连接到元数据列表中的链表 */
	...
	struct dm_array_info info;	   		/* 缓存数组信息 */
	struct dm_array_info hint_info;   	/* 提示数组信息 */
	struct dm_disk_bitset discard_info;	/* 丢弃位图信息 */

	struct rw_semaphore root_lock;		/* 用于保护根块的读写信号量 */
	unsigned long flags;
	dm_block_t root;					/* 缓存元数据的根块 */
	dm_block_t hint_root;				/* 提示元数据的根块 */
	dm_block_t discard_root;			/* 丢弃元数据的根块 */
	 ...
	 */
	__u8 metadata_space_map_root[SPACE_MAP_ROOT_SIZE];  /* 元数据空间映射根块缓冲区 */
	 ...
	 * Metadata format 2 fields.
	 */
	dm_block_t dirty_root;					/* 脏数据的根块 */
	struct dm_disk_bitset dirty_info;		/* 脏数据信息 */	
...
{
	int r;
	// czs：创建一个块管理器对象，用于管理底层块设备上的块，提供读取和写入块的功能
 ...
 */
// czs：开始事务，mutator用于更新超级块标志，然后块管理器的更改刷新到持久设备上
...
/*----------------------------------------------------------------*/
// czs：创建新的缓存元数据对象并初始化，然后创建持久的数据对象，最后开始事务
 ...
 */
// czs：定义了一个用于管理缓存元数据对象的全局列表
static DEFINE_MUTEX(table_lock);
static LIST_HEAD(table);

// czs：查找块设备对应的缓存元数据结构体
...
// czs：查找或打开缓存元数据对象，首先在全局列表中查找，如果找到则返回，否则打开块设备上的缓存元数据（创建）
// 成功打开后，再次查找全局列表，如果找到则销毁刚才打开的缓存元数据并返回，否则将打开的元数据缓存添加到全局列表中
